    ...
    if (str.length == 0) {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                document.getElementById("company").value = myObj[3];
                document.getElementById("dp_name").value = myObj[4];
                document.getElementById("sc_name").value = myObj[5];
                document.getElementById("sc_code").value = myObj[6];
                document.getElementById("sc_type").value = myObj[7];
                document.getElementById("CostCenter_ID").value = myObj[8];
                document.getElementById("shift").value = myObj[9];
            }
        };

        xmlhttp.open(
            "GET",
            "../assets/ajax/autofill_form3.php?Emp_ID=" + str,
            true
        );
...
$(document).ready(function() {
    ...
    $("#save").click(function() {
        ...
        if ($("#mts_company").val() == "") {
            error_mts_company = "*";
            $("#error_mts_company").text(error_mts_company);
            $("#mts_company").css("border-color", "#dd191d");
            mts_company = "";
        } else {
            error_mts_company = "";
            $("#error_mts_company").text(error_mts_company);
            $("#mts_company").css("border-color", "");
            mts_company = $("#mts_company").val();
        }
        if ($("#mts_dp_name").val() == "") {
            error_mts_dp_name = "*";
        ...
        if (
            ...
            if ($("#save").text() == "Add") {
                ...
                output +=
                    "<label>บริษัท: <a>" +
                    mts_company +
                    ' <input type="hidden" name="hidden_mts_company[]" id="mts_company' +
                    count +
                    '" class="mts_company" value="' +
                    mts_company +
                    '" /></a></label>';
                output +=
                    "<p>" +
                    company +
                    '</a><input type="hidden" name="hidden_company[]" id="company' +
                    count +
                    '" class="company" value="' +
                    company +
                    '" /></p>';
            ...
            $("#mts_psn_gr").val("");
            $("#mts_pon_name").val("");
            $("#mts_company").val(null).trigger('change');
    ...
    $(document).on("click", ".view_details", function() {
        ...
        var mts_company = $("#mts_company" + row_id + "").val();
    ...
    $("#FORM3").on("submit", function(event) {
        ...
        if ($("#CHQ1").val() == "") {
            ...
            $.ajax({
                url: "../insert/create_formC.php",
                method: "POST",
                data: form_data,
                success: function(data) {
                    $("#TB3").find("tr:gt(0)").remove();
                    $('#approve').modal('hide');
